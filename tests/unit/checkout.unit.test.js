...
describe("Cart test", () => {
  ...
  describe("Checkout", () => {
    ...
    it("should throw 400 error if address is not set - when User.hasSetNonDefaultAddress() returns false", async () => {
      expect(userTwo.address).toEqual(config.default_address); // will give Errorrr
    ...
    it("should throw 400 error if wallet balance is insufficient", async () => {
      ...
      const userOneWithZeroBalance = { ...userOne, walletMoney: 0 }; // copying the obj & updating the walletMoney
...
//** */
      expect(true).toEqual(userOneFinal.walletMoney < userOne.walletMoney);
