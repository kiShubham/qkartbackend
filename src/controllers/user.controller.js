...
const getUser = catchAsync(async (req, res) => {
  const { email } = req.user; // getting from token
  const { userId } = req.params;
  const { q } = req.query;
  if (q === "address") {
    const userData = await userService.getUserAddressById(userId); //getting address
    ...
    const user = await userService.getUserById(userId); // getting user form userId
    if (user.email !== email) {
      throw new ApiError(httpStatus.FORBIDDEN, "you are not authorized");
    }
    if (!user) {
